...
def a_giou_3d(boxes_a: dict, boxes_b: dict) -> Tuple[np.array, np.array]:
    """
    ...
    # calu gioubev/giou3d and mask invalid value
    a_gioubev = inter_areas / union_areas - (convex_areas - union_areas) / convex_areas
    a_giou3d = inter_volumes / union_volumes - (convex_volumes - union_volumes) / convex_volumes
    a_giou3d[bool_mask], a_gioubev[bool_mask] = -np.inf, -np.inf

    return a_gioubev, a_giou3d

def a_giou_bev(boxes_a: dict, boxes_b: dict) -> np.array:
    """
...
def a_iou_bev(boxes_a: dict, boxes_b: dict) -> np.array:
    """
    ...
    # process norm bottom corners, size for parallel computing
    rep_norm_bcs_a = norm_bcs_a[:, None, :].repeat(len(norm_bcs_b), 1)  # [a_num, b_num, 4]
    rep_norm_bcs_b = norm_bcs_b[None, :, :].repeat(len(norm_bcs_a), 0)  # [a_num, b_num, 4]
    bboxa_area = (rep_norm_bcs_a[:, :, 2] - rep_norm_bcs_a[:, :, 0]) * (rep_norm_bcs_a[:, :, 3] - rep_norm_bcs_a[:, :, 1])
    bboxb_area = (rep_norm_bcs_b[:, :, 2] - rep_norm_bcs_b[:, :, 0]) * (rep_norm_bcs_b[:, :, 3] - rep_norm_bcs_b[:, :, 1])
